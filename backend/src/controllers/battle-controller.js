...
export const getBattleInfo = async (attack_log_id) => {
  const [attack_log] = await query("SELECT * FROM `attack_log` WHERE `id`=?", [
    attack_log_id,
  ]);
  if (!attack_log) {
    return undefined;
  ...
  let [computer_info] = await query(
    ...
    [attack_log.tegenstanderid]
  );

  const enemyPokemons = await query(
    "SELECT id,leven FROM `pokemon_wild_gevecht` WHERE `attack_log_id`= ?",
    [attack_log_id]
  );
