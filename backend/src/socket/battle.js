...
export const Battle = async(socket) => {
    socket.on("InitBattle", async (attack_log_id, callback) => {
        const {computer_info, pokemon_info, attack_log, enemyPokemons} = await getBattleInfo(attack_log_id);
        callback({computer_info,pokemon_info,attack_log,enemyPokemons})
    })
    socket.on('currentBattle', async(callback) => {
        const userId = socket.user?.user_id;
        const [logId] = await query(
          "SELECT `id` FROM `attack_log` WHERE `user_id`=?",
          [userId]
        );
        if (logId) {
          return callback({
